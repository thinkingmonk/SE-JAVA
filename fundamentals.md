...
## Mini project idea (from the video)

**Number guessing game (1–100)**

1. Pick `int secret = (int)(Math.random() * 100) + 1;`
2. Loop: ask user to guess with `Scanner`
3. If guess &lt; secret → “too small”; if greater → “too large”; if equal → “correct” and `break`
4. If user enters a negative number → stop the game

Combines: `Scanner`, loops, `if`/`else`, `Math.random()`, `break`.
...
After fundamentals: **classes & objects**, **constructors**, **inheritance**, **interfaces**, **collections** (`ArrayList`, `HashMap`), and **file I/O**. The same video series and [Oracle Java tutorials](https://docs.oracle.com/javase/tutorial/java/) are good follow-ups.

Practice by editing `Fundamentals/src/main/java/org/fullstack/Main.java` and running from IntelliJ (green Run button).
